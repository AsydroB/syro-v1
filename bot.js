...
client.on("message", (message) => {
  ...
  if(message.content.toLowerCase() === "naber") {
       message.channel.send("ben iyiyim seni sormalı")
   }
...
embed.setTitle("Sunucudan Attılar Beni")
embed.addField("Sunucu adı", guildName)
embed.addField("Sunucu üye sayısı", guildMemberCount)
embed.addField(`Sunucu sahibi`, guildOwnerTag)
embed.addField("Şuan ki Kullanıcı : ",
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  if (newState.id !== client.user.id && newState.voice.channel && newState.voice.channel.id === 784832128210305084) {
    googleTTS("Syro Bot Hoş Geldin!", "tr", 1).then(soylenecek => {
...
//random girince meşaj
client.on('guildCreate', guild => {
    let syro = guild.channels.filter(c => c.type === "text").random()
    syro.send("Beni sunucuna eklediğin için tişikkürler");
});
